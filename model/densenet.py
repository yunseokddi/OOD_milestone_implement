...
class BottleneckBlock(nn.Module):
    def __init__(self, in_planes, out_plans, drop_rate=0.0):
        super(BottleneckBlock, self).__init__()
        inter_planes = out_plans * 4
        self.bn1 = nn.BatchNorm2d(in_planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv1 = nn.Conv2d(in_planes, inter_planes, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn2 = nn.BatchNorm2d(inter_planes)
        self.conv2 = nn.Conv2d(inter_planes, out_plans, kernel_size=3, stride=1, padding=1, bias=False)
        self.dropRate = drop_rate
...
class DenseNet3(nn.Module):
    def __init__(self, depth, num_classes, growth_rate=12, reduction=0.5, bottleneck=True, dropRate=0.0,
                 normalizer=None, out_classes=100):
